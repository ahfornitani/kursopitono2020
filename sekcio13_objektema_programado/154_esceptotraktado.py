...
class NetrovitaTasko(Exception):
    pass
...
class RipetaTasko(Tasko):
    def __init__(self, priskribo, limdato, tagokvanto=7):
        super().__init__(priskribo, limdato)
        self.tagokvanto = tagokvanto
        self.posedanto = None

    def fini(self):
        super().fini()
        nova_limdato = datetime.now() + timedelta(days=self.tagokvanto)
        nova_tasko = RipetaTasko(self.priskribo, nova_limdato, self.tagokvanto)
        if self.posedanto:
...
def main():
    ...
    hejmo += RipetaTasko("Ŝanĝi litotukojn", datetime.now(), 7)
    hejmo.serĉi("Ŝanĝi litotukojn").fini()
    print(hejmo)

    try:
        hejmo.serĉi("Lavi telerojn - ERARO").fini()
    except NetrovitaTasko as e:
        print(f'Kaŭzo estis "{str(e)}"!')
    finally:
        print("Ĉiam okazos!")
